...
import 'package:video_call_app/screens/reset_password_screen/reset_password_screen.dart';
...
class VerifyOTPScreenController extends GetxController {
  ...
  Future<void> loadOTPSendedData() async {
    ...
    userEmailAddress.value =
        prefs.getString(Constants.OTP_SENDER_EMAIL_KEY) ?? '';
  ...
  Future<void> submitResendOTPRequest() async {
    try {
      final response =
          await UserServices.requestUserOTP(userEmailAddress.value);
      ...
      if (response['statusCode'] == 200) {
        ...
        await prefs.setString(
            Constants.OTP_SENDER_EMAIL_KEY, response['data']['requestedEmail']);
