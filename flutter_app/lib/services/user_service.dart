...
class UserServices {
  ...
  static dynamic creatUserAccount(Map<String, dynamic> accInformation) async {
    try {
      final response = await dio.post(
        '${Constants.baseUrl}/users/register',
        data: accInformation,
      );
  ...
  static dynamic updateUserAccount(Map<String, dynamic> accInformation) async {
    try {
      ...
      final response = await dio.patch(
          '${Constants.baseUrl}/users/update-user-account-details',
          data: accInformation,
          options: Options(headers: {'Authorization': 'Bearer $accessToken'}));
  ...
  static dynamic requestPasswordChangeForNotLoggedInUser(
  ...
  static dynamic requestPasswordChangeForLoggedInUser(
      {required Map<String, String> details}) async {
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final accessToken = prefs.getString(Constants.USER_ACCESS_TOKEN);
  ...
  static dynamic requestUserOTP(String email) async {
    try {
      final response =
          await dio.post('${Constants.baseUrl}/users/send-otp', data: {
        "email": email,
      });
  ...
  static dynamic makeACall(String userPhoneNumber) async {
    try {
      ...
      final response =
          await dio.post('${Constants.baseUrl}/calls/create-incoming-call',
              data: {
  ...
  static dynamic changeCallStatusToReject(String callId) async {
    try {
      ...
      final response =
          await dio.patch('${Constants.baseUrl}/calls/change-call-status',
              data: {"callId": callId, "newCallStatus": "REJECTED"},
              options: Options(headers: {
                'Authorization': 'Bearer $accessToken',
              }));
