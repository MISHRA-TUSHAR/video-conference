...
import 'package:video_call_app/screens/home_screen/home_screen.dart';
import 'package:video_call_app/services/signaling_service.dart';
import 'package:video_call_app/utils/toasts.dart';

class VideoCallScreenReciever extends StatefulWidget {
  final String adminSocketId;
  const VideoCallScreenReciever({super.key, required this.adminSocketId});

  @override
  State<VideoCallScreenReciever> createState() =>
      _VideoCallScreenRecieverState();
}

class _VideoCallScreenRecieverState extends State<VideoCallScreenReciever> {
  ...
  _setupPeerConnection() async {
    ...
    socket.on('incomming:call2', handleIncomingCall);
    socket.on('peer:nego:needed2', handleNegoNeeded);
    socket.on('admin:ended:call2', handleAdminCallEnded);
  }
  ...
  void handleIncomingCall(data) async {
    ...
    socket.emit('call:accepted2',
        {"to": widget.adminSocketId, "answer": answer.toMap()});
  }

  void handleNegoNeeded(data) async {
    final rtcSDP = data['offer']['sdp'];
    await _rtcPeerConnection!.setRemoteDescription(
        RTCSessionDescription(rtcSDP, data['offer']['type']));
    final answer = await _rtcPeerConnection!.createAnswer({
      "mandatory": {
        "OfferToReceiveAudio": true,
        "OfferToReceiveVideo": true,
    ...
    socket.emit(
        'peer:nego:done2', {"to": data['from'], "answer": answer.toMap()});
  }
